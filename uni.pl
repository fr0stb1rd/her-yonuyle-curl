...
# Set standard output and error to handles UTF-8
...
open(F, "<:encoding(UTF-8)", "$sum");
while(<F>) {
    my $line = $_;
    if($line =~ /^(\s*)\* \[.*\]\(([^\)]*)\)/) {
        my $indent = length($1);
        my $file = $2;
        my $level = $indent / 2; # SUMMARY.md uses 2 spaces per level
        push @files, { name => $file, level => $level };
    }
}
close(F);
...
sub update_anchor {
    ...
    } elsif($anchor_target =~ m/(^.*)(\.png|\.jpg|\.svg)$/ ) {
        #
...
sub include {
    my ($f, $shift_level) = @_;
    my $line;
    open(M, "<:encoding(UTF-8)", "$f") || return;
    my $dir = dirname($f);
        ...
        if($complete_line =~ /^\`\`\`.*/) {
            ...
            if($complete_line =~ /^(#[\#]*) (.*)/) {
                ...
                if($content =~ /([^\{]*)(\{\#(.*)\})/) {
                    ...
                    $final_section_line = "$depth_str $section \{\#$section_id\}\n";
                }
                else {
