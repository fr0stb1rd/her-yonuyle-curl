...
## Curl_handler
  ...
  - `->flags`, protokol hakkında genel motor tarafından farklı muamele görmesini sağlayan ek bilgiler içeren bir bit maskesidir:
    ...
    - `PROTOPT_CLOSEACTION` - bu protokolün bağlantıyı kapatmadan önce yapması gereken eylemleri vardır. Bu bayrak artık kod tarafından kullanılmıyor, ancak yine de bir dizi protokol işleyicisi için ayarlanmış.
    - `PROTOPT_DIRLOCK` - yön kilidi. SSH protokolleri, ana motorun kendisiyle ilgilendiği soket eylemlerinin hangi yönünü sınırlamak için bu biti ayarlar.
    - `PROTOPT_NONETWORK` - ağı kullanmayan bir protokol (`file:` okuma)
    - `PROTOPT_NEEDSPWD` - bu protokol bir parolaya ihtiyaç duyar ve bir tane sağlanmadıkça varsayılan bir tane kullanır
    - `PROTOPT_NOURLQUERY` - bu protokol URL'deki bir sorgu parçasını işleyemez (?foo=bar)
