...
open(F, "<:encoding(UTF-8)", "index-words") ||
    die "no words";
while(<F>) {
    chomp;
    my $w = $_;
...
sub sorting {
    ...
    # Turkish character sorting normalization
    # Map Turkish chars to ASCII equivalents for sorting key
    $s =~ s/Ç/C/g;
    $s =~ s/Ğ/G/g;
    $s =~ s/İ/I/g;
    $s =~ s/Ö/O/g;
